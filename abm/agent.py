...
class SocialGPAgent(CellAgent):
    ...
    def _make_choice(self):
        ...
        if self.model_type in ["SG-ICM", "SG-LCM"]:
            if self.rho_update_rule == "landmarks_corr_2":
            ...
            elif self.rho_update_rule is not None:
                raise ValueError(
                    f"Unknown rho_update_rule '{self.rho_update_rule}'. "
                    "Only 'landmarks_corr_2' is supported."
                )
